...
export async function GET(
  ...
  if (!validSheet(sheet)) return NextResponse.json({ error: "Aba inválida." }, { status: 400 });
  if (!isGoogleSheetsConfigured()) return NextResponse.json({ configured: false, data: [] });
  try {
...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({ error: "Não foi possível salvar na planilha." }, { status: 500 });
  }
}
